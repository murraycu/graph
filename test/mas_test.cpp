...
BOOST_AUTO_TEST_CASE(test0)
{
  ...
  typedef boost::graph::graph_traits<undirected_graph>::edge_descriptor edge_descriptor;
  ...
  mas_edge_connectivity_visitor<undirected_graph, boost::graph::d_ary_heap_indirect<vertex_descriptor, 22, indicesInHeap_type, distances_type, std::greater<weight_type> > >  test_vis(pq);

  boost::graph::maximum_adjacency_search(g,
        boost::graph::weight_map(weights).
        visitor(test_vis).
  ...
  boost::graph::maximum_adjacency_search(g,
        boost::graph::weight_map(weights).
        visitor(boost::graph::make_mas_visitor(boost::graph::null_visitor())));

  boost::graph::maximum_adjacency_search(g,
        boost::graph::weight_map(weights));

  boost::graph::maximum_adjacency_search(g,
        boost::graph::root_vertex(*vertices(g).first));

  test_vis.clear();
  boost::graph::maximum_adjacency_search(g,
      boost::graph::weight_map(boost::graph::make_constant_property<edge_descriptor>(weight_type(1))).
      visitor(test_vis).
      max_priority_queue(pq));
  BOOST_CHECK_EQUAL(test_vis.curr(), vertex_descriptor(7));
...
BOOST_AUTO_TEST_CASE(test1)
{
  ...
  mas_edge_connectivity_visitor<undirected_unweighted_graph,boost::graph::d_ary_heap_indirect<vertex_descriptor, 22, indicesInHeap_type, distances_type, std::greater<weight_type> > >  test_vis(pq);

  boost::graph::maximum_adjacency_search(g, 
         boost::graph::weight_map(boost::graph::make_constant_property<edge_descriptor>(weight_type(1))).visitor(test_vis).max_priority_queue(pq));
  ...
  boost::graph::maximum_adjacency_search(g, boost::graph::weight_map(ws_map).visitor(test_vis).max_priority_queue(pq));
  BOOST_CHECK_EQUAL(test_vis.curr(), vertex_descriptor(7));
  BOOST_CHECK_EQUAL(test_vis.prev(), vertex_descriptor(6));
