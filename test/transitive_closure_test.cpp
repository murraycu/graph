      ...
      if (*i == *j) {
        if (g_has_edge) {
          ...
          for (boost::tie(k, k_end) = adjacent_vertices(*i, g); k != k_end; ++k) {
            ...
            if (is_reachable(*k, *i, g, boost::make_iterator_property_map(color_map_vec.begin(), boost::get(boost::graph::vertex_index, g)))) {
              can_reach = true;
              break;
            }
          }
        ...
        std::vector<default_color_type> color_map_vec(num_vertices(g));
        if (is_reachable(*i, *j, g, boost::make_iterator_property_map(color_map_vec.begin(), boost::get(boost::graph::vertex_index, g)))) {
          if (num_tc != 1)
            return false;
        } else {
          if (num_tc != 0)
...
bool test(int n, double p)
{
...

  {
    ...
    transitive_closure(g1, g1_tc, vertex_index_map(boost::get(boost::graph::vertex_index, g1)));
  }
  ...
  if(check_transitive_closure(g1, g1_tc))
    ...
    print_graph(g1, boost::get(boost::graph::vertex_index, g1));
    cout << "Result is ";
    print_graph(g1_tc, boost::get(boost::graph::vertex_index, g1_tc));
    return false;
  }
}
