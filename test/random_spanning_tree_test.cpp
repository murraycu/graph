...
void write_spanning_tree(const Graph& g, PredMap pred, WeightMap weight, string filename) {
  boost::shared_array_property_map<string, typename boost::property_map<Graph, edge_index_t>::const_type> edge_style(num_edges(g), boost::get(edge_index, g));
  boost::shared_array_property_map<string, typename boost::property_map<Graph, vertex_index_t>::const_type> vertex_pos(num_vertices(g), boost::get(vertex_index, g));
...
int main(int, char**) {
  ...
  boost::shared_array_property_map<gt::vertex_descriptor, boost::property_map<graph_type, vertex_index_t>::const_type> pred(num_vertices(g), get(vertex_index, g));
  boost::shared_array_property_map<double, boost::property_map<graph_type, edge_index_t>::const_type> weight(num_edges(g), get(edge_index, g));
