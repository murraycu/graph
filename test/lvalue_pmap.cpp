...
{
  boost::property_map<graph_t, vertex_info_t>::type pmap
    = boost::get(vertex_info_t(), x);
  return pmap[vertex(0, x)];
}

const double& foo_2(graph_t const & x)
{
  boost::property_map<graph_t, vertex_info_t>::const_type pmap
    = boost::get(vertex_info_t(), x);
  return pmap[vertex(0, x)];
}
...
{
  boost::property_map<graph_t, edge_info_t>::type pmap
    = boost::get(edge_info_t(), x);
  return pmap[edge(vertex(0, x), vertex(1, x), x).first];
}

const double& bar_2(graph_t const & x)
{
  boost::property_map<graph_t, edge_info_t>::const_type pmap
    = boost::get(edge_info_t(), x);
  return pmap[edge(vertex(0, x), vertex(1, x), x).first];
}
