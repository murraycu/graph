      ...
      const detail::stored_ra_edge_iter<Vertex, EdgeVec, Property>& e)
  {
    ...
    namespace detail {
                                             ...
                                             boost::allow_parallel_edge_tag)
      {
        ...
        BOOST_STATIC_ASSERT((!std::is_same<EdgeListS, vecS>::value));
    ...
    edges(const directed_edges_helper<Config>& g_)
    {
      ...
      const auto& cg = static_cast<const graph_type&>(g_);
      auto& g = const_cast<graph_type&>(cg);
    ...
    namespace detail {
      ...
      struct remove_undirected_edge_dispatch<no_property> {
              ...
              no_property&)
        {
          ...
          typedef typename Config::graph_type graph_type;
          auto& g = static_cast<graph_type&>(g_);
          auto p = (no_property*)e.get_property();
          auto& out_el = g.out_edge_list(source(e, g));
          auto out_i = out_el.begin();
          typename Config::EdgeIter edge_iter_to_erase;
          for (; out_i != out_el.end(); ++out_i)
                               ...
                               boost::disallow_parallel_edge_tag)
      {
        ...
        typedef typename EdgeList::value_type StoredEdge;
        auto i = el.find(StoredEdge(v)), end = el.end();
        if (i != end) {
          g.m_edges.erase((*i).get_iter());
          el.erase(i);
        }
    ...
    {
      auto sv = (typename C::stored_vertex*)v;
           ...
           const undirected_graph_helper<Config>& g_)
    {
      ...
      const auto& g = static_cast<const Graph&>(g_);
                 ...
                 bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      auto& in_el = in_edge_list(g, u);
      auto in_ei = in_el.begin(), in_ei_end = in_el.end();
                   ...
                   bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      auto& g = static_cast<graph_type&>(g_);
      auto& in_el = in_edge_list(g, u);
      auto in_ei = in_el.begin();
      auto in_ei_end = in_el.end();
      for (; in_ei != in_ei_end; ++in_ei) {
        detail::erase_from_incidence_list
          (g.out_edge_list((*in_ei).get_target()), u, Cat());
        g.m_edges.erase((*in_ei).get_iter());
    ...
    {
      typedef typename Config::graph_type AdjList;
      const auto& g = static_cast<const AdjList&>(g_);
         ...
         const adj_list_helper<Config, Base>& g_)
    {
      ...
      typedef typename Config::StoredEdge StoredEdge;
      const auto& cg = static_cast<const Graph&>(g_);
      const auto& el = cg.out_edge_list(u);
      auto it = graph_detail::
    ...
    namespace detail {
      ...
      get_dispatch(const adj_list_helper<Config, Base>& g, Property p,
        ...
        typedef typename Config::graph_type Graph;
        typedef typename boost::property_map<Graph, Property>::const_type PA;
        const auto& cg = static_cast<const Graph&>(g);
        return PA(&cg, p);
      }
    ...
    {
      typedef typename Config::graph_type Graph;
      auto pmap = get(p, static_cast<Graph&>(g));
      put(pmap, key, value);
    }
    ...
    public:
      ...
      {
        auto* v = new vertex_descriptor[num_vertices];
        for (auto i = 0; i < num_vertices; ++i)
      ...
      {
        const auto& x = static_cast<const Derived&>(x_);
    ...
    {
      const auto& g = static_cast<const Derived&>(g_);
    ...
    namespace detail {
                             ...
                             boost::undirected_tag)
      {
        ...
        auto ei = g.m_edges.begin(), ei_end = g.m_edges.end();
        for (; ei != ei_end; ++ei) {
          if (ei->m_source > u)
            --ei->m_source;
          if (ei->m_target > u)
    ...
    public:
      ...
      {
        const auto& x = static_cast<const Graph&>(x_);
        // Copy the stored vertex objects by adding each vertex
        // and copying its property object.
        for (auto i = 0; i < num_vertices(x); ++i) {
          auto v = add_vertex(*this);
    ...
    {
      BOOST_USING_STD_MAX();
      auto x = max BOOST_PREVENT_MACRO_SUBSTITUTION(u, v);
  ...
  namespace detail {
        ...
        >
    {
      ...
      inline Ref operator[](key_type e) const {
        auto& p = *(Property*)e.get_property();
