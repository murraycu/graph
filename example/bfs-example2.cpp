...
main()
{
  ...
  using graph_t = boost::adjacency_list<boost::listS, boost::listS, boost::undirectedS,
    VertexProps>;
  ...
  for (std::size_t i = 0; i < N; ++i)
    verts.emplace_back(boost::add_vertex(g));
  for (std::size_t j = 0; j < n_edges; ++j)
    boost::add_edge(verts[edge_array[j].first], verts[edge_array[j].second], g);
#else
  ...
  boost::breadth_first_search(g, vertex(s, g), color_map(boost::get(&VertexProps::color, g)).
    visitor(vis));

  // a vector to hold the discover time property for each vertex
  std::vector<Size> dtime(boost::num_vertices(g));
  using dtime_pm_type =
  ...
  std::vector<boost::graph_traits<graph_t>::vertices_size_type > discover_order(N);
