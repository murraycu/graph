...
struct scaled_closeness_measure
{
    ...
    {
        if(d == boost::numeric_values<Distance>::infinity()) {
            return boost::numeric_values<Result>::zero();
        }
        else {
            return div(Result(boost::num_vertices(g)), Result(d));
        }
    }
    Divide div;
};
...
using DistanceProperty = boost::exterior_vertex_property<Graph, int>;
...
using WeightMap = boost::constant_property_map<Edge, int>;
...
main(int argc, char *argv[])
{
    ...
    ClosenessContainer cents(boost::num_vertices(g));
