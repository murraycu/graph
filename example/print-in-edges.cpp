                ...
                VertexNameMap name_map)
{
  ...
  for (const auto& edge : make_range_pair(in_edges(v, g)))
    out << boost::get(name_map, source(edge, g)) << " -> "
      << boost::get(name_map, target(edge, g)) << std::endl;
}
...
main()
{
  ...
  i = std::find_if(i, end, name_equals("libzigzag.a", boost::get(vertex_name, g)));
  output_in_edges(std::cout, g, *i, boost::get(vertex_name, g));
